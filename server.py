...
def remove_validation_keywords_from_schema(
    schema: Any, in_properties_map: bool = False
) -> Any:
    """
    Recursively removes all specified validation keywords from a JSON schema,
    leaving only the structural elements.

    Important: Do NOT treat the contents of an object's `properties` map as
    validation keywords. This prevents legitimate parameter names like
    "pattern" from being deleted when they are property names.
    """
    if isinstance(schema, dict):
        # If we're currently visiting the value of a `properties` field, this dict
        # is the mapping of property_name -> property_schema. Do not delete keys here.
        if in_properties_map:
            for prop_name, prop_schema in list(schema.items()):
                schema[prop_name] = remove_validation_keywords_from_schema(
                    prop_schema, in_properties_map=False
                )
            return schema

        # First, remove any validation keywords at the current schema level
        ...
        for key, value in list(schema.items()):
            if key == "properties" and isinstance(value, dict):
                # Enter properties map context to avoid deleting property names like "pattern"
                schema[key] = remove_validation_keywords_from_schema(
                    value, in_properties_map=True
                )
            else:
                schema[key] = remove_validation_keywords_from_schema(
                    value, in_properties_map=False
                )

    elif isinstance(schema, list):
        # If it's a list, recurse into each item
        return [
            remove_validation_keywords_from_schema(item, in_properties_map=False)
            for item in schema
        ]
...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    for msg in anthropic_request.messages:
        # Handle ASSISTANT messages (potential tool_use)
        if msg.role == "assistant":
            ...
            # Ensure content is a list before iterating
            ...
            for block in content_list:
                if hasattr(block, "type"):
                    if block.type == "text":
                        ...
                        has_tool_use = True
                        # Arguments must be a JSON string for LiteLLM
                        tool_calls.append(
                            {
                                "id": block.id,
                                "type": "function",
            ...
            if text_content:
                ...
                # Use empty string for content when there are tool calls to avoid placeholder injection
        ...
        # Handle USER messages (potential tool_result)
        elif msg.role == "user":
            if isinstance(msg.content, str):
            ...
            # Process all blocks in the user message
            for block in content_list:
                if hasattr(block, "type"):
                    if block.type == "text":
                        text_content += block.text + "\n"
    ...
    try:
        ...
        if request.model.startswith("openai/"):
            ...
            if LLM_BASE_URL:
                ...
                logger.debug(
                    f"Using LLM API key and custom base URL {LLM_BASE_URL} for model: {request.model}"
                )
            else:
        ...
        if "openai" in litellm_request["model"] and "messages" in litellm_request:
            ...
            for i, msg in enumerate(litellm_request["messages"]):
                ...
                if "content" in msg and isinstance(msg["content"], list):
                    ...
                    if is_only_tool_result and len(msg["content"]) > 0:
                        ...
                        # Replace the list with extracted text (empty string if nothing)
                ...
                if "content" in msg:
                    ...
                    if isinstance(msg["content"], list):
                        ...
                        # Allow empty string; don't inject placeholders
            ...
            for i, msg in enumerate(litellm_request["messages"]):
                ...
                if isinstance(msg.get("content"), list):
                    ...
                    logger.warning(
                        f"Message {i} has None content - replacing with empty string"
                    )
                    litellm_request["messages"][i]["content"] = ""
        ...
        if request.stream:
            logger.error(
                f"--- STREAMING LITELLM REQUEST: {json.dumps(litellm_request)} ---"
            )
            ...
            logger.error(
                f"--- NON-STREAMING LITELLM REQUEST: {json.dumps(litellm_request)} ---"
            )
